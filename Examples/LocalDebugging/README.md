...
`LocalLambdaServer started and listening on 127.0.0.1:8000, receiving events on /invoke`
which means the local emulator is up and receiving traffic on port `8000` and expecting events on the `/invoke` endpoint.
